...
A utilização de ferramentas de Inteligência Artificial Generativa ao longo desta etapa ocorreu de forma colaborativa, principalmente como suporte técnico para a validação da sintaxe e da semântica UML, especialmente na estruturação dos diagramas de estados e na definição das regras de transição e dos pseudoestados de escolha. A IA também foi utilizada na prototipação inicial de arquivos PlantUML (`.puml`) e no refinamento da nomenclatura de eventos e ações.
